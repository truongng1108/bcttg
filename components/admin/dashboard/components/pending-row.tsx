...
export function PendingRow({ title, type, author, date, status }: PendingRowProps) {
  const statusConfig = PENDING_STATUS_LABELS[status]
  ...
  return (
    <tr className="transition-colors hover:bg-muted/30">
      ...
      <td className="px-4 py-3">
        <span
          className={`inline-flex rounded px-2 py-0.5 text-xs font-medium ${statusConfig.className}`}
        >
          {statusConfig.label}
        </span>
      </td>
